...
- **Blindagem de Transmissão UTF-8 via WhatsApp**:
  - Reescrita total da alocação de Emojis dentro de `notifications.py`. Ao invés de trafegar emojis injetados à seco em string (causa comum de bugs de losango na abertura do WhatsApp), forcei localmente chaves Unicode hexadecimais explícitas (ex: `\U0001F534` para 🔴).
  - Execução de passe mandatória `quote(raw_msg.encode('utf-8'))` forçando o parse bruto para a ISO oficial da Meta, estabilizando quebras de linha paramétricas `%0A`.
